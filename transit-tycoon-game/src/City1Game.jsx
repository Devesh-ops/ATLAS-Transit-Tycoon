...
const CITY_META = {
  ...
  population: 50000,
  intro: `A city of 50,000 people. Everyone travels by ride-hailing or bus. You have a $12M annual budget. You have two levers: tax Uber rides (raises revenue, reduces congestion, but hurts mobility) and subsidize buses (costs money, boosts mobility, always cuts congestion). Balance the tradeoffs — and watch the clock.`,
};
...
const ADVISOR = {
  ...
  monthStartHints: [
    ...
    "Bus subsidies always boost mobility and reduce congestion — a strong lever throughout the year.",
    ...
    "Bus subsidies always ease congestion and boost mobility — they're your citizen welfare tool.",
  ...
  tooltips: {
    ...
    mobility:    "How much citizens are moving. Uber tax reduces it (fewer affordable rides). Bus subsidy always helps mobility.",
    ...
    busSubsidy:  "Discount on bus fares. Costs budget. Boosts mobility and always reduces congestion.",
  },
};
...
function simulate(uberTax, busSubsidy, budgetRemaining) {
  ...
  // 1. Mobility — Uber tax reduces mobility, bus subsidy always boosts it
  ...
  return { mobilityScore, congestionLevel, happinessScore, monthlyDelta, uberRevenue, busCost, budgetStress, revenuePositive };
}
...
// ============================================================
//  CITY BACKGROUND VISUALIZATION
// ============================================================
function CityBackground({ stats, subtle = false }) {
  const { mobilityScore = 60, congestionLevel = 45, happinessScore = 50 } = stats || {};

  // Sky color based on happiness (gradient from dark/polluted to bright/clear)
  const skyColors = happinessScore >= 65
    ? ['#87CEEB', '#B0E0E6'] // happy: bright blue sky
    : happinessScore >= 45
    ? ['#9CA3AF', '#D1D5DB'] // neutral: gray sky
    : ['#6B7280', '#9CA3AF']; // unhappy: dark gray/polluted sky

  // Congestion affects road density and color
  const roadColor = congestionLevel >= 60 ? '#DC2626' : congestionLevel >= 35 ? '#F59E0B' : '#10B981';

  // Number of vehicles based on congestion and mobility
  const numCars = Math.floor(congestionLevel / 8) + 3;
  const numBuses = Math.floor(mobilityScore / 15) + 1;

  // Building colors based on happiness
  const buildingColors = happinessScore >= 65
    ? ['#FCD34D', '#FBBF24', '#F59E0B', '#FB923C'] // warm, vibrant
    : happinessScore >= 45
    ? ['#94A3B8', '#64748B', '#475569', '#334155'] // neutral grays
    : ['#52525B', '#3F3F46', '#27272A', '#18181B']; // dark, depressed

  const opacity = subtle ? 0.2 : 0.5;
  ...
  return (
    <div style={{
      ...
      <div style={{
        position: 'absolute',
        inset: 0,
        background: `linear-gradient(180deg, ${skyColors[0]} 0%, ${skyColors[1]} 100%)`,
      ...
      <div style={{
        ...
        {[...Array(8)].map((_, i) => (
          <div key={`building-${i}`} style={{
            ...
            background: buildingColors[i % buildingColors.length],
            borderRadius: '2px 2px 0 0',
            position: 'relative',
            transition: 'background 1s ease, height 0.8s ease',
            boxShadow: '0 -2px 10px rgba(0,0,0,0.2)'
          }}>
            {/* Windows */}
            {happinessScore >= 45 && [...Array(3)].map((_, j) => (
              <div key={`window-${i}-${j}`} style={{
                position: 'absolute',
                top: `${20 + j * 25}%`,
                left: '20%',
                width: '60%',
                height: '15%',
                background: Math.random() > 0.3 ? '#FEF3C7' : '#0F172A',
                opacity: 0.8,
                borderRadius: '1px',
      ...
      <div style={{
        ...
        <div style={{
          ...
          {[...Array(numCars)].map((_, i) => (
            <div key={`car-${i}`} style={{
              width: '8px',
              height: '5px',
              background: congestionLevel >= 60 ? '#DC2626' : '#EF4444',
              borderRadius: '1px',
              opacity: 0.7,
              animation: `moveVehicle ${8 + Math.random() * 4}s linear infinite`,
        ...
        <div style={{
          ...
          {[...Array(numBuses)].map((_, i) => (
            <div key={`bus-${i}`} style={{
              width: '14px',
              height: '7px',
              background: '#3B82F6',
              borderRadius: '1px',
              opacity: mobilityScore >= 50 ? 0.8 : 0.5,
              animation: `moveVehicle ${10 + Math.random() * 5}s linear infinite reverse`,
      ...
      {/* Floating particles (pollution or leaves based on happiness) */}
      <div style={{
        position: 'absolute',
        inset: 0
      }}>
        {[...Array(happinessScore >= 60 ? 6 : 3)].map((_, i) => (
          <div key={`particle-${i}`} style={{
            position: 'absolute',
            left: `${Math.random() * 100}%`,
            top: `${Math.random() * 60}%`,
            width: happinessScore >= 60 ? '3px' : '2px',
            height: happinessScore >= 60 ? '3px' : '2px',
            background: happinessScore >= 60 ? '#10B981' : '#6B7280',
            borderRadius: '50%',
            opacity: 0.4,
...
function AdvisorBox({ message }) {
  return (
    <div style={{ background: "rgba(5,13,26,0.9)", backdropFilter: "blur(10px)", border: "1px solid #0C1E30", borderRadius: 10, padding: "11px 13px", display: "flex", gap: 10, alignItems: "flex-start" }}>
...
function PlanningScreen({ month, roundIndex, uberTax, busSubsidy, onUberChange, onBusChange, onCommit, budgetRemaining }) {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#020817", fontFamily: "Georgia, serif", padding: "16px", outline: warn ? "3px solid rgba(239,68,68,0.3)" : "3px solid transparent", transition: "outline 0.3s", position: "relative" }}>
      <CityBackground stats={live} subtle={false} />
      ...
      <div style={{ maxWidth: 580, margin: "0 auto", position: "relative", zIndex: 10 }}>
        ...
        {/* Sliders */}
        <div style={{ background: "rgba(5,14,26,0.85)", backdropFilter: "blur(8px)", border: warn ? "1px solid rgba(239,68,68,0.4)" : "1px solid #0C2340", borderRadius: 11, padding: "16px 16px 4px", marginBottom: 10, transition: "border 0.3s" }}>
        ...
        {/* Gauges */}
        <div style={{ background: "rgba(5,14,26,0.85)", backdropFilter: "blur(8px)", border: "1px solid #0C2340", borderRadius: 11, padding: "14px 16px", marginBottom: 12 }}>
...
function ResultScreen({ month, roundIndex, stats, uberTax, busSubsidy, advisorMessage, onNext, history, timedOut, budgetRemaining }) {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#020817", fontFamily: "Georgia, serif", padding: "16px", position: "relative" }}>
      <CityBackground stats={stats} subtle={false} />
      <div style={{ maxWidth: 580, margin: "0 auto", position: "relative", zIndex: 10 }}>
        ...
        <div style={{ background: "rgba(5,14,26,0.85)", backdropFilter: "blur(8px)", border: "1px solid #0C2340", borderRadius: 11, padding: "14px 16px", marginBottom: 10 }}>
        ...
        <div style={{ background: "rgba(5,14,26,0.85)", backdropFilter: "blur(8px)", border: "1px solid #0C2340", borderRadius: 8, padding: "10px 14px", marginBottom: 10, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
