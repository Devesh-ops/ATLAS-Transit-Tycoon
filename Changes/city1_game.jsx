...
function simulate(uberTax, busSubsidy, budgetRemaining, roundIndex = 12) {
  ...
  return {
    ...
    happinessBreakdown: [
      ...
      ...(hBudg < 0 ? [{ label: "Budget", value: hBudg, color: C.red }] : [])
    ]
  };
}
  ...
  return (
    <div style={{
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
        ...
        <div style={{ fontSize: 16, fontWeight: 800, color: C.textSub }}>{goalGrade} or above to Advance</div>
      </div>
    </div>
  );
}
...
function PlanningScreen({ month, roundIndex, uberTax, busSubsidy, onUberChange, onBusChange, onCommit, budgetRemaining, history }) {
  ...
  return (
    <div style={{ minHeight: "100vh", background: C.pageBg, fontFamily: "Georgia,serif", outline: warn ? `3px solid ${C.red}` : "3px solid transparent", outlineOffset: "-3px", transition: "outline 0.3s" }}>
      ...
      <div style={{ maxWidth: 1320, margin: "0 auto", padding: "16px" }}>

        {/* Header */}
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
          <div>
            <div style={{ fontSize: 9, letterSpacing: 3, color: C.blue, textTransform: "uppercase", fontWeight: 800 }}>City 1 · {CITY_META.name}</div>
            <h2 style={{ fontSize: 26, fontWeight: 800, color: C.text, margin: "3px 0 0" }}>{month}</h2>
          </div>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <div style={{ textAlign: "right" }}>
              <div style={{ fontSize: 9, color: C.textFaint }}>Month</div>
              <div style={{ fontSize: 15, fontWeight: 800, color: C.textSub }}>{roundIndex + 1}/12</div>
            </div>
            <CountdownRing timeLeft={timeLeft} total={TIMER.monthDuration} />
          </div>
        </div>

        <div style={{ height: 4, background: C.track, borderRadius: 2, marginBottom: 14, overflow: "hidden" }}>
        ...
        <div style={{ display: "flex", gap: 12, alignItems: "stretch" }}>
          <div style={{ width: 240, flexShrink: 0 }}>
            {/* AV Lockout Banner */}
            {uberLocked && (
              <div style={{ background: C.redBg, border: `1px solid ${C.redBorder}`, borderRadius: 9, padding: "11px 14px", marginBottom: 10, display: "flex", gap: 10, alignItems: "flex-start" }}>
                <span style={{ fontSize: 20, flexShrink: 0 }}>🚗</span>
                <div>
                  <div style={{ fontSize: 11, fontWeight: 800, color: C.red, textTransform: "uppercase", letterSpacing: 1, marginBottom: 3 }}>
                    Uber / AV Tax Unavailable
                  </div>
                  <div style={{ fontSize: 12, color: C.textSub, lineHeight: 1.5 }}>
                    Tax unlocks in <strong>March</strong>.
                  </div>
                </div>
              </div>
            )}
            ...
            {/* Policy card */}
            <div style={{ background: C.cardBg, border: `1px solid ${warn ? C.red : C.border}`, borderRadius: 12, padding: "18px 18px 8px", boxShadow: "0 1px 6px rgba(0,0,0,0.06)", transition: "border 0.3s" }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ background: C.cardBg, border: `1px solid ${C.border}`, borderRadius: 12, overflow: "hidden", boxShadow: "0 1px 6px rgba(0,0,0,0.06)", height: "100%" }}>
              <div style={{ height: "100%", minHeight: 360, position: "relative" }}>
                <CityRoadScene
                  cityLevel={1}
                  uberTax={uberTax}
          ...
          <div style={{ width: 240, flexShrink: 0, display: "flex", flexDirection: "column" }}>
            {/* Live preview */}
            <div style={{ background: C.cardBg, border: `1px solid ${C.border}`, borderRadius: 12, padding: "14px 14px 10px", marginBottom: 12, boxShadow: "0 1px 6px rgba(0,0,0,0.06)", maxHeight: 520, overflowY: "auto" }}>
              <div style={{ fontSize: 11, fontWeight: 800, color: C.textMuted, textTransform: "uppercase", letterSpacing: 1, marginBottom: 12, position: "sticky", top: 0, background: C.cardBg, paddingBottom: 6 }}>Live Preview</div>
              <GaugeBar
                label="Happiness"
            ...
            <button onClick={() => commitMonth(false)} disabled={locked}
              style={{ width: "100%", background: locked ? C.border : C.green, color: locked ? C.textMuted : "#fff", border: "none", borderRadius: 10, padding: "14px", fontSize: 16, fontWeight: 800, cursor: locked ? "not-allowed" : "pointer", transition: "background 0.2s", marginTop: "auto" }}>
