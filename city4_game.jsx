...
function diagnoseRun(history, finalBudget) {
  ...
  if (avgIE < 50) {
    failures.push({
      ...
      body: `Average income equity was ${Math.round(avgIE)}. In Crestwood, Uber taxes primarily hit the wealthy — but you didn't reinvest enough into the poor. Average bus subsidy was only ${Math.round(avgB)}% — failing to sliderage the top-down funding for the bottom-up need.`,
...
function PlanningScreen({ month, roundIndex, uberTax, busSubsidy, acLevel,
  ...
  const projectedHistory = [...history, { cityHappiness: live.cityHappiness, genderEquityScore: live.genderEquityScore, incomeEquityScore: live.incomeEquityScore }];
...
export default function CrestwoodTycoonCity4({ onAdvance, onSetFinalScreen }) {
  ...
  // Load save on mount
  useEffect(() => {
    const saved = localStorage.getItem("atlas_save_city4");
    if (saved) {
      try {
  ...
  useEffect(() => {
    ...
    localStorage.setItem("atlas_save_city4", JSON.stringify(state));
  ...
  const handleRestart = useCallback(() => {
    localStorage.removeItem("atlas_save_city4");
